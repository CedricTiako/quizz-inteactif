...
async function loadAndDisplayStats() {
  ...
  try {
    const userIdT =
      (await fetchUserIdByPhone(localStorage.getItem("phone"))) ?? 1;
    console.log("userIdT:", userIdT);
    const response = await axios.get(
      `${API_BASE_URL}?lang=${preferredLang}&endpoint=user_stats&user_id=${userIdT}`
    );
...
async function loadQuestions(phone) {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}?userid=${userIdT}&lang=${preferredLang}&endpoint=questions`
    );
...
async function loadAnswers(questionId) {
  try {
    const response = await axios.get(
      `https://ayoba-yamo-quizz.zen-apps.com/api/index.php?lang=${preferredLang}&endpoint=answers&question_id=${questionId}`
    );
    ...
    answers.forEach((answer) => {
      ...
      answerButton.addEventListener("click", () =>
        handleAnswer(answer.answer_id, answer.is_correct, answerButton)
      );
...
async function loadTickets() {
  ...
  try {
    const response = await axios.get(
      `${API_BASE_URL}?lang=${preferredLang}&endpoint=get_all_tickets`
    );
...
async function createUser(phone, usernameInput = "") {
  ...
  try {
    ...
    const response = await axios.post(
      `${API_BASE_URL}?lang=${preferredLang}&endpoint=users`,
      formData,
      {
        headers: {
  ...
  if (container) {
    container.innerHTML = `
            <div class="text-center py-12 animate__animated animate__fadeIn">
                ...
                <h2 class="text-3xl font-bold gradient-text mb-4">${t(
                  "quiz_complete"
                )}</h2>
...
function createConfetti() {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    particle.style.backgroundColor = ["#4F46E5", "#7C3AED", "#F59E0B"][
...
function updateRewardsProgress(currentPoints) {
  ...
  if (!progressBar || !progressText || !rewardsContainer || !tickets.length)
    return;
  ...
  const currentTicket =
    sortedTickets.filter((t) => currentPoints >= t.points_required).pop() ||
    sortedTickets[0];
  const nextTicketIndex =
    sortedTickets.findIndex((t) => t.id === currentTicket.id) + 1;
  const nextTicket =
    nextTicketIndex < sortedTickets.length
      ? sortedTickets[nextTicketIndex]
      : null;
  ...
  if (!nextTicket) {
    ...
    const pointsNeededForNextLevel =
      nextTicket.points_required - previousTicketPoints;
...
*/
  rewardsContainer.innerHTML = sortedTickets
    .map(
      (ticket) => `
        <div class="reward-item ${
          currentPoints >= ticket.points_required ? "opacity-100" : "opacity-50"
        } 
...
async function loadRewards() {
  try {
    const response = await axios.get(
      `https://ayoba-yamo-quizz.zen-apps.com/api/index.php?lang=${preferredLang}&endpoint=get_all_tickets`
    );
    ...
    const totalPoints = Math.max(
      ...rewards.map((reward) => parseInt(reward.points_required, 10))
    );
    ...
    rewards.forEach((reward) => {
      ...
      rewardElement.className =
        "reward-item p-4 border-b flex items-start space-x-4";
...
function processAnswerFeedback(isCorrect, pointsAwarded) {
  if (isCorrect) {
    ...
    showFeedback(
      t("correct_answer_feedback").replace("{points}", pointsAwarded),
      "success"
    );
    //  createConfetti();
...
async function loadRewardData() {
  try {
    ...
    const response = await axios.get(
      "https://ayoba-yamo-quizz.zen-apps.com/api/index.php?lang=fr&endpoint=get_all_tickets"
    );
    ...
    console.error(
      "Erreur lors du chargement des données de récompense:",
      error
    );
  }
}
...
function createMediaModal_old(mediaUrl) {
  ...
  modalContent.className =
    "bg-white rounded-lg shadow-lg overflow-hidden max-w-3xl w-full relative";

  const closeButton = document.createElement("button");
  closeButton.className =
    "absolute top-4 right-4 text-gray-700 bg-gray-200 rounded-full p-2 hover:bg-gray-300";
...
function createMediaModal_2(mediaUrl) {
  // Vérifier si un modal existe déjà et le supprimer
  const existingModal = document.getElementById("mediaModal");
  if (existingModal) {
    existingModal.remove();
...
function callApi() {
  fetch(
    `https://ayoba-yamo-quizz.zen-apps.com/api/index.php?lang=fr&endpoint=set_tickets&user_id=${userId}`,
    {
      method: "GET",
    }
  )
...
function checkPoints() {
  ...
  const pointsRequired = parseInt(
    document.getElementById("points_required").innerText,
    10
  );
